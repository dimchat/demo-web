(function () {
    ...
    var A  = uint256(0)
    var B  = uint256(7)
    var GX = uint256("79BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798", 16)
    var GY = uint256("483ADA7726A3C4655DA4FBFC0E1108A8FD17B448A68554199C47D08FFB10D4B8", 16)
    var P  = uint256("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F", 16)
    var N  = uint256("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141", 16)
    //var RED = BN.red(P)
    var _0 = uint256(0)
    var _1 = uint256(1)
    ...
    function ecsign(d, z) {
        while (true) {
            ...
            var R = mulG(k)
            if (R[0] == 0) continue
            var s = mulmod(invmod(k, N), addmod(z, mulmod(R[0], d, N), N), N)
            if (s == 0) continue
    ...
    function JtoA(p) {
        var zInv = invmod(p[2], P)
        var zInv2 = mulmod(zInv, zInv, P)
    ...
    function ecdouble(_p) {
        ...
        var z2 = mulmod(_p[2], _p[2], P)
        var m = addmod(mulmod(A, mulmod(z2, z2, P), P), mulmod(uint256(3), mulmod(_p[0], _p[0], P), P), P)
        var y2 = mulmod(_p[1], _p[1], P)
        var s = mulmod(uint256(4), mulmod(_p[0], y2, P), P)

        var x = addmod(mulmod(m, m, P), negmod(mulmod(s, uint256(2), P), P), P)
        return [
            x,
    ...
    function ecadd(_p, _q) {
        ...
        var z2 = mulmod(_q[2], _q[2], P)
        var u1 = mulmod(_p[0], z2, P)
        var s1 = mulmod(_p[1], mulmod(z2, _q[2], P), P)
        z2 = mulmod(_p[2], _p[2], P)
        var u2 = mulmod(_q[0], z2, P)
        var s2 = mulmod(_q[1], mulmod(z2, _p[2], P), P)
        ...
        z2 = mulmod(u2, u2, P)
        var t2 = mulmod(u1, z2, P)
        z2 = mulmod(u2, z2, P)
        s2 = addmod(s2, negmod(s1, P), P)
        var x = addmod(addmod(mulmod(s2, s2, P), negmod(z2, P), P), negmod(mulmod(uint256(2), t2, P), P), P)
        return [
            x,
    ...
    function isValidPoint(x, y) {
        var yy = addmod(mulmod(mulmod(x, x, P), x, P), B, P)
    ...
    function decompressKey(x, yBit) {
        var redP = BN.red('k256');
        x = x.toRed(redP)
        var y = x.redMul(x).redMul(x).redAdd(B.toRed(redP)).redSqrt()
        var sign = y.testn(0)
    ...
    function ecrecover(recId, sigr, sigs, message) {
        ...
        //   1.1 Let x = r + jn
        var x = addmod(uint256(sigr), P.muln(recId >> 1), P)
        ...
        var y = decompressKey(x, (recId & 1) == 1)
        //   1.4. If nR != point at infinity, then do another iteration of Step 1 (callers responsibility).
        // if (!R.mul(N).isInfinity())
        //     return null
        //   1.5. Compute e from M using Steps 2 and 3 of ECDSA signature verification.
        var e = uint256(message)
        ...
        var eNeg = negmod(e, N)
        var rInv = invmod(sigr, N)
        var srInv = mulmod(rInv, sigs, N)
        var eNegrInv = mulmod(rInv, eNeg, N)
        var R = AtoJ(x, y)
        var G = AtoJ(GX, GY)
        var qinJ = ecadd(ecmul(G, eNegrInv), ecmul(R, srInv))
        var p = JtoA(qinJ)
        return {x: toHex(p[0]), y: toHex(p[1])}
    }
    ...
    function ecverify (Qx, Qy, sigr, sigs, z) {
        var w = invmod(sigs, N)
        var u1 = mulmod(z, w, N)
        var u2 = mulmod(sigr, w, N)
        var Q = AtoJ(Qx, Qy)
        var G = AtoJ(GX, GY)
        var RinJ = ecadd(ecmul(G, u1), ecmul(Q, u2))
        var r = JtoA(RinJ)
        return sigr.eq(r[0])
    }
